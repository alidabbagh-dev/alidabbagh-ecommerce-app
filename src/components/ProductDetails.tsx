          ...
          <div className="w-full flex flex-col justify-center">
            <div className="container flex flex-col-reverse lg:flex-row items-center gap-4 p-4">
              <div className="w-full lg:w-2/4">
                ...
                <h4 className="font-semibold text-green-500">{product.brand}</h4>
                ...
                <span className="text-xl text-gray-300 pl-4 line-through">{product.price}$</span>
                <div>
                  <button
                    onClick={() => {
              ...
              <div className="w-full">
                <div className="relative aspect-video w-full">
                  <Image
                    ...
                    width={500}
                    height={500}
                    src={imageToShow < 0 ? product.thumbnail : product.images[imageToShow]}
                    alt=""
                  />
                  <div className="absolute w-full top-2/4 translate-y-[-50%] flex justify-between px-3">
                    <button onClick={() => handleSelectImage("left")} className="arrow-btn main-btn">
                      <GoArrowLeft />
                    </button>
                    <button onClick={() => handleSelectImage("right")} className="arrow-btn main-btn">
                      <GoArrowRight />
                    </button>
                  </div>
                </div>
                <div className="w-full flex justify-center mt-4">
                  <>
                    <button onClick={() => setImageToShow(-1)}>
                      {imageToShow == -1 ? <RxDotFilled size={30} /> : <RxDot size={30} />}
                    </button>
                    {product.images.map((_, index) => (
                      <button key={index} onClick={() => setImageToShow(index)}>
                        {imageToShow == index ? <RxDotFilled size={30} /> : <RxDot size={30} />}
                      </button>
                    ))}
                  </>
                </div>
